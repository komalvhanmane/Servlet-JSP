...
<body>
	<nav class="navbar navbar-dark bg-dark">
		<div class="container-fluid">
			...
			<form action="HistoryController">
				<button type="submit" class="btn btn-outline-info" value="war"
					name="filter">Search History</button>
			</form>
			<form action="LogoutController">
			<button type="submit"
					class="btn btn-outline-info">Logout</button>
					</form>
		</div>
	</nav>
	...
	<nav class="navbar navbar-expand-lg navbar-light bg-light">
		<div class="container-fluid">
			...
			<form autocomplete="off" action="SearchedMovieController">
				<div class="autocomplete" style="width: 300px;">
					<input id="myInput" type="text" placeholder="Search Movie"
						name="searchmovie" required>
				</div>
				<input name="email2" value= "<%=email%>" type="hidden">
					...
					<div class="p-3 border bg-light">
						...
						<div class="container">
							<div class="row row-cols-2 row-cols-lg-3 g-2 g-lg">
								<%
								LinkedList<Movie> lst = (LinkedList<Movie>) session.getAttribute("similarmoviename");
								%>
								<%
								if (lst != null) {
								%>
								...
								%>
								<h3>NOT FOUND</h3>
								<img alt="" src="./assets1/Not_found.jpg">
								<%
								}
								int count=0;
								...
								<div class="col">
									<div class="p-3 border bg-light">
										<div class="card" style="width: 18rem;">
											<img src="./assets1/hello-i-m-nik-cKcPF4aQBFE-unsplash.jpg"
												class="card-img-top" alt="...">
											<div class="card-body">
												<h5 class="card-title">
													Movie Name ::
													<%=lst.get(i).getName()%></h5>
												<p class="card-text">
													Genres ::
													<%=lst.get(i).getGenres()%></p>
												Cast ::
												<%=lst.get(i).getCast() %>
											</div>
										</div>
			...
			<div class="row gx-5">
				<div class="col">
					<div class="p-3 border bg-light">
						...
						<form action="recommendedMovie">
							<button type="submit" class="btn btn-outline-info" name="recommend" value="recommend">Recommend</button>
						</form>
						<div class="container">
							<div class="row row-cols-2 row-cols-lg-3 g-2 g-lg">
								<%
								LinkedList<Movie> lst2 = (LinkedList<Movie>) session.getAttribute("similarmovie");
								%>
								<%
								if (lst2 != null) {
								%>
									...
									<div class="p-3 border bg-light">
										<div class="card" style="width: 18rem;">
											...
											<div class="card-body">
												<h5 class="card-title">
													Movie Name ::
													<%=lst2.get(i).getName()%></h5>
												<p class="card-text">
													Genres ::
													<%=lst2.get(i).getGenres()%></p>
												Cast ::
												<%=lst2.get(i).getCast() %>
	...
	//const API_KEY = '2b5cabf80643a494399746b96314764f';
	//const BASE_URL ='https://api.themoviedb.org/3/movie/';
	//const url=BASE_URL+API_KEY;
	function getMoviesImages(){
		fetch('https://api.themoviedb.org/3/movie/65/images?api_key=2b5cabf80643a494399746b96314764f&language=en-US').then(res => res.json()).then(data =>{
			console.log(data);
		})
	}
	</script>
